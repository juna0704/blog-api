...
const deleteCurrentUser = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const publicdIds = blogs.map(({ banner }) => banner.publicId);
    await cloudinary.api.delete_resources(publicdIds);
    logger.info('Multiple blogbanners deleted from cloudinary', {
      publicdIds,
    });
